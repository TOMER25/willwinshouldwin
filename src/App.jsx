...
function HomeScreen({ onSelectShow, user, onGoProfile, onGoAdmin, onGoHistory, onGoExplorer, allShows }) {
  return (
        ...
        <div className="shows-section shows-section--history">
          ...
          <div className="shows-grid">
            <button className="show-card show-card-history" onClick={onGoHistory}>
              <div className="show-card-top">
                <span className="show-badge badge-history">98 Ceremonies</span>
              </div>
            ...
            <button className="show-card show-card-explorer" onClick={onGoExplorer}>
              <div className="show-card-top">
                <span className="show-badge badge-explorer">98 Years of Data</span>
              </div>
              <h3 className="show-name">Oscar Explorer</h3>
              <p className="show-org">Films · People · Categories</p>
              <p className="show-cats">Filter by category, decade, wins, and nominations</p>
              <div className="show-cta show-cta--gold">Search the history →</div>
            </button>
          </div>
        </div>
...
function PicksExportCard({ show, picks, winners, displayName, willColor, shouldColor, onClose }) {
  ...
  const generate = () => {
    ...
    // Layout constants (logical px — canvas rendered at 2x for hi-res)
    const SCALE        = 2;
    const W            = 800;
    const PADDING      = 36;
    const COL_GAP      = 24;
    const COL_W        = (W - PADDING * 2 - COL_GAP) / 2;
    const HALF_W       = COL_W / 2 - 8;
    const HEADER_H     = 178;   // taller to fit logo + name
    const FOOTER_H     = 64;
    const LINE_H       = 18;
    const CAT_H        = 18;
    const ROW_PAD      = 14;
    ...
    const mCtx = tmp.getContext("2d");  // no scale applied

    // Pre-measure: compute each row's height from real wrapped line counts
    const rowHeights = categories.map(cat => {
      const p = picks[cat.id] || {};
      mCtx.font = F_PICK;
      const wLines = wrapText(mCtx, p.will_win   ? "\u2605 " + p.will_win   : "", HALF_W);
      const sLines = wrapText(mCtx, p.should_win ? "\u2665 " + p.should_win : "", HALF_W);
      const maxLines = Math.max(wLines.length || 1, sLines.length || 1);
      return CAT_H + maxLines * LINE_H + ROW_PAD;
    });

    const colH = [0, 0];
    for (let i = 0; i < categories.length; i++) {
      colH[i < ROWS_PER_COL ? 0 : 1] += rowHeights[i];
    }
    const BODY_H = Math.max(colH[0], colH[1]);
    const H      = HEADER_H + BODY_H + FOOTER_H;

    // ── Set up hi-res canvas ──
    ...
    bar.addColorStop(0, willColor);
    bar.addColorStop(1, shouldColor);
    ctx.fillStyle = bar;
    ctx.fillRect(0, 0, W, 5);

    // ── Logo: "★ WillWin / ♥ ShouldWin" ──
    const logoY = 46;
    ctx.textAlign = "center";
    ...
    const totalLogoW = starW + wwW + sepW + heartW + swW;
    ...
    const legendY = HEADER_H - 24;
    ctx.font = F_LGND;
    ctx.textAlign = "left";
    for (const cx of [leftX, rightX]) {
      ctx.fillStyle = willColor;   ctx.fillText("\u2605  Will Win",    cx,              legendY);
      ctx.fillStyle = shouldColor; ctx.fillText("\u2665  Should Win",  cx + HALF_W + 10, legendY);
    }

    // Column divider
    ctx.strokeStyle = "#1e2540"; ctx.lineWidth = 1;
    ctx.beginPath();
    ctx.moveTo(W / 2, HEADER_H - 4); ctx.lineTo(W / 2, HEADER_H + BODY_H + 4);
    ctx.stroke();

    // Header divider
    ctx.strokeStyle = "#2a3050"; ctx.lineWidth = 1;
    ctx.beginPath();
    ctx.moveTo(PADDING, HEADER_H - 10); ctx.lineTo(W - PADDING, HEADER_H - 10);
    ctx.stroke();
    ...
    categories.forEach((cat, i) => {
      ...
      const x    = colXs[col];
      const y    = HEADER_H + colY[col];
      const rowH = rowHeights[i];
      colY[col] += rowH;

      const p    = picks[cat.id]   || {};
      const wp   = p.will_win      || null;
      const sp   = p.should_win    || null;
      const win  = winners[cat.id] || null;
      const wHit = !!(win && wp === win);
      const sHit = !!(win && sp === win);
      const rowIdx = col === 0 ? i : i - ROWS_PER_COL;

      // Alternating tint
      if (rowIdx % 2 === 0) {
        ctx.fillStyle = "rgba(255,255,255,0.018)";
        ctx.fillRect(col === 0 ? 0 : W / 2, y, W / 2, rowH);
      }
      // Correct-pick highlight
      if (wHit) {
        ctx.fillStyle = willColor + "1a";
        ctx.fillRect(col === 0 ? 0 : W / 2, y, W / 2, rowH);
        ctx.strokeStyle = willColor + "33"; ctx.lineWidth = 1;
        ctx.strokeRect((col === 0 ? 0 : W / 2) + 0.5, y + 0.5, W / 2 - 1, rowH - 1);
      }

      // Row bottom border
      ctx.strokeStyle = "#1a2035"; ctx.lineWidth = 1;
      ctx.beginPath();
      ctx.moveTo(col === 0 ? 0 : W / 2, y + rowH);
      ctx.lineTo(col === 0 ? W / 2 : W,  y + rowH);
      ctx.stroke();

      // Category label
      ctx.font = F_CAT; ctx.fillStyle = "#505870"; ctx.textAlign = "left";
      ctx.fillText(cat.name.toUpperCase(), x, y + CAT_H - 3);

      // Pick zone divider
      ctx.strokeStyle = "#1e2540"; ctx.lineWidth = 1;
      ctx.beginPath();
      ctx.moveTo(x + HALF_W + 6, y + CAT_H);
      ctx.lineTo(x + HALF_W + 6, y + rowH - 4);
      ctx.stroke();

      const pickY  = y + CAT_H + LINE_H - 2;
      const shouldX = x + HALF_W + 12;

      // Will Win — wrap using unscaled mCtx for measurement
      mCtx.font = wHit ? F_BPICK : F_PICK;
      ctx.font   = wHit ? F_BPICK : F_PICK;
      ctx.fillStyle = wp ? (wHit ? willColor : willColor + "cc") : "#2e3450";
      ctx.textAlign = "left";
      const wLines = wp ? wrapText(mCtx, "\u2605 " + wp, HALF_W) : ["\u2605  \u2014"];
      if (!wp) { ctx.fillStyle = "#2e3450"; ctx.font = F_PICK; }
      wLines.forEach((ln, li) => ctx.fillText(ln, x, pickY + li * LINE_H));

      // Should Win — wrap using unscaled mCtx for measurement
      mCtx.font = sHit ? F_BPICK : F_PICK;
      ctx.font   = sHit ? F_BPICK : F_PICK;
      ctx.fillStyle = sp ? (sHit ? shouldColor : shouldColor + "cc") : "#2e3450";
      const sLines = sp ? wrapText(mCtx, "\u2665 " + sp, HALF_W) : ["\u2665  \u2014"];
      if (!sp) { ctx.fillStyle = "#2e3450"; ctx.font = F_PICK; }
      sLines.forEach((ln, li) => ctx.fillText(ln, shouldX, pickY + li * LINE_H));
    });
    ...
    if (resultsMode) {
      ...
      ctx.fillStyle = shouldColor; ctx.fillText("\u2665 " + sc + "/" + tot, PADDING + 80, footY + 40);
      ctx.font = F_FOOT; ctx.fillStyle = "#3a4060"; ctx.textAlign = "right";
      ctx.fillText("willwinshouldwin.com", W - PADDING, footY + 40);
    } else {
      ctx.font = F_FOOT; ctx.fillStyle = "#3a4060"; ctx.textAlign = "center";
      ctx.fillText("willwinshouldwin.com", W / 2, footY + 40);
    }
  };
  ...
  return (
    <div className="export-card-wrap">
      <canvas ref={canvasRef} style={{ width: "100%", borderRadius: "8px", border: "1px solid var(--border)", display: "block" }} />
      <div style={{ display: "flex", gap: "0.75rem", marginTop: "0.75rem" }}>
        <button className="results-card-btn" style={{ flex: 1 }} onClick={download}>Download image</button>
        <button className="results-close-btn" style={{ flex: 1 }} onClick={onClose}>Done</button>
      </div>
            ...
            {resultsOpen.__outer && gradedShows.map(({ show: s, willCorrect, shouldCorrect, graded }) => {
              ...
              return (
                <div key={s.id} className="results-accordion-item">
                  ...
                  >
                    <span className="results-acc-name">{s.name} ({s.year})</span>
...
function HistoricalPicksSection({ userId }) {
  ...
  const loadPicks = async () => {
    const { data } = await supabase
      ...
      .select("ceremony, category, nominee_index, updated_at")
      .eq("user_id", userId)
      .order("ceremony", { ascending: false });
    if (!data || data.length === 0) { setLoading(false); return; }
    setPicks(data);
    const ceremonies = [...new Set(data.map(p => p.ceremony))];
  ...
  const matchedWinner = picks.filter(p => nominees[`${p.ceremony}-${p.category}-${p.nominee_index}`]?.winner).length;
  const alignmentPct = totalPicks > 0 ? Math.round((matchedWinner / totalPicks) * 100) : null;
      ...
      {alignmentPct !== null && (
        <div className="hist-stats-strip">
          <div className="hist-stat">
            <span className="hist-stat-num">{totalPicks}</span>
            <span className="hist-stat-label">Picks logged</span>
          </div>
          <div className="hist-stat">
            <span className="hist-stat-num" style={{ color: "var(--crimson)" }}>{ceremonyNums.length}</span>
            <span className="hist-stat-label">Ceremonies</span>
          </div>
          <div className="hist-stat">
            <span className="hist-stat-num" style={{ color: "var(--gold)" }}>{alignmentPct}%</span>
            <span className="hist-stat-label">Matched winner</span>
          </div>
        </div>
      )}
        ...
        {ceremonyNums.map(num => {
          ...
          return (
            <div key={num} className="hist-picks-ceremony">
              <button className="hist-ceremony-toggle" onClick={() => setOpenCeremonies(o => ({ ...o, [num]: !o[num] }))}>
                <span className="hist-ceremony-label">{ordinal(num)} Academy Awards</span>
                <span className="hist-ceremony-meta">
                  ...
                  {catPicks.map(p => {
                    ...
                    return (
                      <div key={p.category} className="hist-pick-row">
                        ...
                        <span className="hist-pick-name">♥ {label}{nom.winner && <span className="hist-pick-match"> · matched winner ✓</span>}</span>
                      </div>
                    );
                  })}
                </div>
...
const CEREMONY_LIST = (() => {
  ...
  const list = [];
  for (let i = 98; i >= 1; i--) list.push({ ceremony: i, year: yearMap[i] });
  return list;
})();
...
const CAT_SHORT = {
  ...
  "Animated Short Film":          "Anim. Short",
  "Live Action Short Film":       "Live Short",
...
function OscarExplorerScreen({ onGoHome, onGoHistory }) {
  ...
  const [filmMode, setFilmMode] = useState("AND");
  ...
  const [personType, setPersonType] = useState("wins");
    ...
    <div className="explorer-screen">
      <div className="explorer-header">
        ...
        <div className="explorer-title-block">
          <h1 className="explorer-title">Oscar <span style={{ color: "var(--gold)" }}>Explorer</span></h1>
          <p className="explorer-subtitle">Search 98 years of Academy Award history</p>
        </div>
      </div>

      <div className="explorer-tabs">
        <button className={`explorer-tab ${tab === "films" ? "active" : ""}`} onClick={() => setTab("films")}>Films</button>
        <button className={`explorer-tab ${tab === "people" ? "active" : ""}`} onClick={() => setTab("people")}>People</button>
      </div>
      ...
      {loading ? (
        ...
        <div className="explorer-body">
          <div className="explorer-presets">
            ...
            {FILM_PRESETS.map(p => (
              <button key={p.label} className="explorer-preset-chip" onClick={() => applyFilmPreset(p)}>{p.label}</button>
            ))}
          </div>
          ...
          <div className="explorer-filters">
            <div className="explorer-filter-row">
              ...
              <div className="explorer-filter-group">
                <label className="explorer-filter-label">Decade</label>
                <div className="explorer-decade-row">
                  <select className="explorer-select" value={filmDecadeFrom} onChange={e => setFilmDecadeFrom(e.target.value)}>
                    <option value="">From</option>
                    {EXPLORER_DECADES.map(d => <option key={d} value={d}>{d}</option>)}
                  </select>
                  <span className="explorer-decade-sep">–</span>
                  <select className="explorer-select" value={filmDecadeTo} onChange={e => setFilmDecadeTo(e.target.value)}>
                    <option value="">To</option>
                    {EXPLORER_DECADES.map(d => <option key={d} value={d}>{d}</option>)}
                  </select>
                </div>
              </div>
              {hasFilmFilters && <button className="explorer-reset-btn" onClick={resetFilm}>Reset</button>}
            </div>
          ...
          {!hasFilmFilters ? (
            ...
            <div className="explorer-empty">
              <p className="explorer-empty-text">No films match these filters.</p>
              <button className="explorer-reset-btn" onClick={resetFilm}>Reset filters</button>
            </div>
          ) : (
            <div className="explorer-results">
              <p className="explorer-results-count">{sortedFilmResults.length} film{sortedFilmResults.length !== 1 ? "s" : ""}</p>
              <div className="explorer-table-wrap">
                <table className="explorer-table">
                  <thead>
                    <tr>
                      <th className="explorer-th" onClick={() => toggleSort("title", setFilmSort)}>Film <SortArrow col="title" sort={filmSort} /></th>
        ...
        <div className="explorer-body">
          <div className="explorer-presets">
            ...
            {PEOPLE_PRESETS.map(p => (
              <button key={p.label} className="explorer-preset-chip" onClick={() => applyPersonPreset(p)}>{p.label}</button>
            ))}
          </div>
          ...
          <div className="explorer-filters">
            <div className="explorer-filter-row">
              ...
              <div className="explorer-filter-group">
                <label className="explorer-filter-label">Decade</label>
                <div className="explorer-decade-row">
                  <select className="explorer-select" value={personDecadeFrom} onChange={e => setPersonDecadeFrom(e.target.value)}>
                    <option value="">From</option>
                    {EXPLORER_DECADES.map(d => <option key={d} value={d}>{d}</option>)}
                  </select>
                  <span className="explorer-decade-sep">–</span>
                  <select className="explorer-select" value={personDecadeTo} onChange={e => setPersonDecadeTo(e.target.value)}>
                    <option value="">To</option>
                    {EXPLORER_DECADES.map(d => <option key={d} value={d}>{d}</option>)}
                  </select>
                </div>
              </div>
              {hasPersonFilters && <button className="explorer-reset-btn" onClick={resetPerson}>Reset</button>}
            </div>
          ...
          {!hasPersonFilters ? (
            ...
            <div className="explorer-empty">
              <p className="explorer-empty-text">No people match these filters.</p>
              <button className="explorer-reset-btn" onClick={resetPerson}>Reset filters</button>
            </div>
          ) : (
            <div className="explorer-results">
              ...
              <div className="explorer-table-wrap">
                <table className="explorer-table">
                  <thead>
                    <tr>
                      <th className="explorer-th" onClick={() => toggleSort("name", setPersonSort)}>Name <SortArrow col="name" sort={personSort} /></th>
                      <th className="explorer-th" onClick={() => toggleSort("wins", setPersonSort)}>Wins <SortArrow col="wins" sort={personSort} /></th>
                      <th className="explorer-th" onClick={() => toggleSort("noms", setPersonSort)}>Noms <SortArrow col="noms" sort={personSort} /></th>
                      <th className="explorer-th">Categories</th>
                      <th className="explorer-th">Years</th>
                    </tr>
                  </thead>
                  <tbody>
...
function HistoryScreen({ user, onGoHome }) {
  ...
  const [nominees, setNominees]       = useState([]);
  const [shouldPicks, setShouldPicks] = useState({});
  const [wonGuesses, setWonGuesses]   = useState({});
  ...
  const loadHistStats = async () => {
    const { data } = await supabase
      .from("historical_picks").select("ceremony, category, nominee_index").eq("user_id", user.id);
  ...
  const selectCeremony = async (num) => {
    setSelected(num);
    setNominees([]); setShouldPicks({}); setWonGuesses({}); setRevealed({}); setPickCounts({});
    const [nomRes, pickRes, countRes] = await Promise.all([
      supabase.from("historical_ceremonies").select("*").eq("ceremony", num).order("category").order("nominee_index"),
  ...
  return (
    <div className="app">
      ...
      {!selected && (
        <div className="app-main">
          ...
          {histStats && histStats.total > 0 && (
            <div className="hist-browser-stats">
              <div className="hist-stat">
                <span className="hist-stat-num">{histStats.total}</span>
                <span className="hist-stat-label">Should Wins logged</span>
              </div>
              <div className="hist-stat">
                <span className="hist-stat-num" style={{ color: "var(--crimson)" }}>{histStats.ceremonies}</span>
                <span className="hist-stat-label">Ceremonies visited</span>
              </div>
              <div className="hist-stat">
                <span className="hist-stat-num" style={{ color: "var(--gold)" }}>
                  {Math.round((histStats.matched / histStats.total) * 100)}%
                </span>
                <span className="hist-stat-label">Taste matched winner</span>
              </div>
            </div>
          )}
              ...
              return (
                <div key={cat} className={`category-card ${inAgreement ? "category-card--agreed" : bothHavePick ? "category-card--disagreed" : ""}`}>
                          ...
                          <div className="nominee-name">
                            ...
                            <span>
                              {nom.name !== nom.film && nom.name
                                ? <><strong>{nom.name}</strong><span style={{ color: "var(--text-dim)", fontSize: "0.82em" }}> — {displayFilm}</span></>
                                : displayFilm}
                            </span>
                            {isRevealed && pct > 0 && <span className="agg-badge should-agg">{pct}%</span>}
                          </div>
                          <div className="nominee-picks">
                            <button className={wonBtnClass} onClick={() => handleWonGuess(cat, nom.nominee_index)}
                              disabled={isRevealed} title={isRevealed ? "Already revealed" : "Guess who won"}>
                              {wonBtnContent}
                            </button>
                            <button className={`pick-btn should-btn ${isShouldPick ? "selected" : ""}`}
                  ...
                  {!isRevealed && myGuess !== undefined && (
                    <button className="history-reveal-btn" onClick={() => revealWinner(cat)}>Reveal winner →</button>
                  )}
                  {isRevealed && (
                    <p className={`history-reveal-result ${guessCorrect ? "correct" : "wrong"}`}>
                      {guessCorrect ? "✓ Correct!" : `✗ The winner was ${actualWinner?.name !== actualWinner?.film && actualWinner?.name ? actualWinner.name : actualWinner?.film}`}
                    </p>
                  )}
                </div>
              );
      ...
      {selected && nominees.length === 0 && (
        <div className="app-main"><div className="loading">Loading ceremony…</div></div>
      )}
  ...
  return (
    <>
      <HomeScreen
        ...
        onGoExplorer={() => setScreen("explorer")}
        allShows={allShows}
      />
