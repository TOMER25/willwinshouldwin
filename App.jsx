...
function PicksExportCard({ show, picks, winners, displayName, willColor, shouldColor, onClose }) {
  ...
  const generate = () => {
    ...
    const HALF_W      = COL_W / 2 - 8;   // width available per pick column
    const HEADER_H    = 148;
    const FOOTER_H    = 64;
    const LINE_H      = 18;              // height per wrapped line
    const CAT_H       = 18;             // category label height
    const ROW_PAD     = 14;             // extra vertical padding per row
    const ROWS_PER_COL = Math.ceil(categories.length / 2);

    // Font strings — no escaped quotes
    const F_CAT   = "10px monospace";
    const F_PICK  = "13px Georgia, serif";
    const F_BPICK = "bold 13px Georgia, serif";
    const F_HDR   = "bold 36px Georgia, serif";
    const F_SHOW  = "11px monospace";
    const F_LGND  = "10px monospace";
    const F_FOOT  = "10px monospace";
    const F_SCORE = "bold 13px Georgia, serif";

    // ── Pre-measure pass: compute each row's height based on real text wrapping ──
    const tmp = document.createElement("canvas");
    tmp.width = 200; tmp.height = 10;
    const mCtx = tmp.getContext("2d");
    // Note: mCtx is NOT scaled — we measure at logical px directly
    mCtx.font = F_PICK;
    ...
    // Legend (one per column)
    const leftX  = PADDING;
    const rightX = W / 2 + COL_GAP / 2 + 2;
    ctx.font = F_LGND;
    ctx.textAlign = "left";
    for (const cx of [leftX, rightX]) {
      ctx.fillStyle = willColor;   ctx.fillText("\u2605  Will Win",   cx,              110);
      ctx.fillStyle = shouldColor; ctx.fillText("\u2665  Should Win", cx + HALF_W + 10, 110);
    }
